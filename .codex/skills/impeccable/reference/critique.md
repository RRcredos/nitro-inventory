...
- If subagents are unavailable, fall back sequentially: finish and record Assessment A first, then run Assessment B, then synthesize.
...
Delegate Assessment A and Assessment B to separate subagents when possible. They must not see each other's output. Do not show findings to the user until synthesis.
