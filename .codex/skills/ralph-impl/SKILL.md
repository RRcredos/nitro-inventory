...
1a. identify the linked implementation plan document from the `links` section (GitHub links under `https://github.com/RRcredos/nitro-inventory-knowledge-base`, per `linear.md`)
...
2a. Read `hack/create_worktree.sh`, then create the worktree using the Linear ticket key, branch name, and base branch **`staging`** (third argument): `./hack/create_worktree.sh NIT-xxxx BRANCH_NAME staging`.
2b. Verify worktree creation succeeded before doing anything else (confirm directory exists at `~/projects/boss-pos-wt/NIT-xxxx` and git context is that worktree).
...
2e. Start the implementation Codex subagent in the worktree path: `~/projects/boss-pos-wt/NIT-xxxx` with command `$implement-plan`.
